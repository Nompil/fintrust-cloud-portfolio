...
| --- | --- | --- |
| SQS queue types and failure handling | Got it | FIFO publisher, redrive configuration, and partial batch failure test |
| SNS fan-out and filtering | Got it | Messaging decision note and encrypted alert topic in CloudFormation |
...
My revision priority is AppSync resolver selection and Lambda networking. I will redraw both request paths from memory, then answer scenario questions that force a choice between NAT, gateway endpoints, interface endpoints, API Gateway WebSocket, and AppSync subscriptions.
